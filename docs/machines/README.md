...
**One artifact inaccuracy was recorded rather than fixed here, and
[#100](https://github.com/nopivnick/lineup-prototype-03/issues/100) has since fixed it.**
`offering.machine.ts`'s `LIVE_STATES` comment read *nine states, not the ten ticket 14
ruled on*; ticket 14 ruled on nine, and the ten was #15's. Rule 1 puts an artifact amendment
behind its own closed ticket and #98 asked for none, so #98 recorded it rather than fixing
it silently, per [#50](https://github.com/nopivnick/lineup-prototype-03/issues/50), and
#100 is that ticket — the comment now credits the ten to #15 and the nine to #14, and says
the two nines are different sets. The state set itself is correct in both files and no rule
turns on it.
